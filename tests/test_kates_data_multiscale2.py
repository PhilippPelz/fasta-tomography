...
fn = '20190909_BoxCd4_Pd_tomo3_preprocessed.h5'

f = h5read(path+fn)

proj = f['stack']
angles_in_deg = f['alpha']
dx = f['dx'] * 1e10

# from skimage.transform import rotate
# for i in trange(proj.shape[0]):
#     proj[i] = rotate(proj[i], 90, resize=False, center=None, order=1, mode='constant', cval=0, clip=True,
#                 preserve_range=False)

angles_in = np.deg2rad(angles_in_deg)
...
#%%
save_stack_movie(path+'gif', pstack, np.rad2deg(angles_in), dx)
...
resolution_cutoff = 0.5
resolutions = np.array([2.5, resolution_cutoff])
...
i_res = 0
stack_res = res_stacks[i_res].copy()
m = 60
...
for i_refine in range(refinement_iterations):
    ...
    for t in range(angle_trials):
        ...
        trial_angles[t] = random_offsets + angles
...
path1 = '/home/philipp/drop/Public/'
np.save(path1 + '20190909_BoxCd4_Pd_tomo3_preprocessed.npy', sol.cpu().numpy())
