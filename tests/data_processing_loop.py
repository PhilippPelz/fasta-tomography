...
#%%
with h5.File(path+fn,'r') as f:
    angles1 = f['data']['raw']['tiltangles'][:]
    proj1 = f['data']['raw']['data'][:,:,:]

with h5.File(path+fn2,'r') as f:
    angles2 = f['data']['raw']['tiltangles'][:]
    proj2 = f['data']['raw']['data'][:, :, :]

print(f"angles1.shape: {angles1.shape}")
print(f"angles2.shape: {angles2.shape}")


proj1 = np.transpose(proj1[:,:,:,0],[2,1,0])
proj2 = np.transpose(proj2[:,:,:,0],[2,1,0])
...
# proj1[63] *= 0.9
# proj1[64] *= 0.9
proj_along_rot_axis = np.sum(proj1,1)
