...
% Inputs
rCut             = 6;
sigmaFind        = [1 0]/2;  % kernel values in pixels  % for 0.5 A pixels, 
minIntensityAdd  = 150;
% Density deletion inputs
radiusNN         = 20;   % in pixels / voxels
minNumNNsAllowed = 15;  % if site has less NNs than this, it will be removed
boundDelete      = 10;
...
%         'symmetric');
% end
% volCorr = volCorr .* (max(volSubtract(:))/max(volCorr(:)));
...
% dxyz(sub,:) = [];
% minIntensityAdd = 0.75 * max(volCorr(:));
% disp([' minIntensityAdd ' num2str(minIntensityAdd) ])
...
p=find_maxima3D(volSubtract,sphere1);
...
r2max = radiusNN^2;
sPeaksNew.numNN = zeros(size(peaksRefineUnion,1));
