function [sPeaks] = initializeAtomTraceHelper(vol, minimumIntensity)
% Colin Ophus - April 2018 - atom tracing for phase contrast reconstructions
% 30 - Find initial candidates for peaks from volume

% inputs
sigmaFind = [1 0]/2;  % kernel values for initial peak search
% minimumIntensity = 170;  % peaks dimmer than this are not included in list
sigmaInit = 4; % initial guess for peak standard deviation in pixels 
boundDelete = 10;
BoxSize1=3;
...
%         'symmetric');
% end
% volCorr = volCorr .* (max(vol(:))/max(volCorr(:)));
